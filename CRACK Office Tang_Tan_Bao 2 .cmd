...
::============================================================================
::
::   Homepage: https://linktr.ee/baotaobao
::      Email: https://linktr.ee/baotaobao
::
::============================================================================



::  To activate Office with TangTanBao_BaoTaoBao, run the script with "/Ohook" parameter or change 0 to 1 in below line
set _act=0

::  To remove TangTanBao_BaoTaoBao, run the script with /Ohook-Uninstall parameter or change 0 to 1 in below line
set _rem=0
...
cls
color 07
title  TangTanBao_BaoTaoBao %masver%
...
echo TangTanBao_BaoTaoBao is supported only on Windows 8/10/11 and their server equivalents.
echo:
...
if %_unattended%==0 (
    ...
    echo:
    echo                 [1] Kich Hoat office vinh Vien
    echo         ____________________________________________________________
    echo: 
    call :dk_color2 %_White% "             " %_Cyan% "Duoc lam boi TangTanBao_BaoTaoBao"
    
    :: Bỏ qua lựa chọn và chuyển thẳng đến menu 2
    goto :oh_menu2
)
...
echo:
echo Tang Tan Bao...                    [C2R ^| %_version% ^| %_oArch%]
...
echo:
echo Tang Tan Bao - BaoTaoBao                    [C2R ^| %_version% %_AudienceData%^| %_oArch%]
...
call :dk_color %Green% "Tang Tan Bao - BaoTaoBao Da kick hoat OK."
...
echo https://linktr.ee/baotaobao
) else (
...
echo Tang Tan Bao - BaoTaoBao         ["%_hookPath%\sppcs.dll"] [Successful]
if defined exhook (
echo Copying Custom %_hook% to            ["%_hookPath%\sppc.dll"] [Successful]
) else (
echo Tang Tan Bao - BaoTaoBao  %_hook% to         ["%_hookPath%\sppc.dll"] [Successful]
)
) else (
set error=1
...
set error=1
set ierror=1
call :dk_color %Red% "Tang Tan Bao - BaoTaoBao  %_hook%     [Failed]"
) else (
echo Tang Tan Bao - BaoTaoBao  %_hook%     [Successful]
)
)
...
echo:
echo Tang Tan Bao - BaoTaoBao                    [MSI ^| %_version% ^| %_oArch%]
...
::  Clear the vNext/shared/device license blocks which may prevent TangTanBao_BaoTaoBao
...
echo Tang Tan Bao - BaoTaoBao          [Successfully cleared from all %counter% user accounts]
...
echo Tang Tan Bao - BaoTaoBao    [Successfully added to all %counter% ^& future new user accounts]
)
...
echo Crack By Tang Tan Bao                        [%winos% ^| %fullbuild% ^| %osarch%]
exit /b
...
echo Tang Tan Bao          %~1 [Successful]
) else (
...
echo Cai Xong Se Tu Dong           [%wpainfo%]
)
...
if defined fixes (
    call :dk_color2 %Blue% "Opening Troubleshoot Page..."
    for %%# in (%fixes%) do (start %%#)
)

if defined terminal (
    call :dk_color %_Yellow% "%_exitmsg%..."
) else (
    call :dk_color %_Yellow% "%_exitmsg%..."
)

timeout /t 2 >nul
start https://linktr.ee/baotaobao
exit /b
